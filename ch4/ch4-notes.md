...
- Loops can be used to count the number of occurrences in a string or verify that a string meets specific criteria
- Counting Matches
  - Can use loops to calculate the number of characters in a string that are uppercase
  - Can sometimes also use the `in` operator
    - for example, to find vowels, use `if char.lower() in "aeiou":`
    - Note that `lower()` is a method used to limit the number of characters in the checked string
```python
string = "Tim"
uppercase = 0
for char in string:
...
```
- Finding all Matches
  - To examine every character in a string, you can use a `for` loop to iterate over the characters of the string
  - If you need to know the position of the characters that meet criteria, you can use a range statement, like below
```python
word = "Tim"
...
- Finding the first or last match
  - When finding matches, you can either find the first or last of a match
  - This can be done by setting a boolean variable along with a for or while loop
  - Strings can also be parsed backwards by using negative indexes
- Validating a String
  - Can use while loops to check that the user has entered data in a specific format, such as a correctly formatted telephone number
- Building a new String
  - Can use loops over a string to replace certain characters and construct a new string as output

## 4.16 Application: Random Numbers and Simulations
- Simulation programs use computers to simulate activities in the real world
- In many simulations, loops are used to modify the state of a system and to observe changes
- Generating Random Numbers
  - Often, it is difficult to predict a precise event, but average behaviors can be more accurately predicted
  - Python can help by generating random numbers through the `random()` function
  - These numbers are not exactly random though, they are *pseudorandom*.
  - These numbers are calculated from mathematical formulas
```python
...
- Simulating Die Tosses
  - If you need to generate random numbers from a specific range, you can use the `randint(a, b)` function
- Monte Carlo Method
  - This is a method for finding approximate solutions to problems that cannot be precisely solved
### Definitions
...
- When facing more complex problems, a good strategy is to break down the task into smaller and simpler problems first
- This can seem daunting at first, but is something that can help make problems more manageable
- 